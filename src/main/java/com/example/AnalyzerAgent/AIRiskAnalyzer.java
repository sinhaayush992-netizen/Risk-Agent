...
class AIRiskAnalyzer {
    ...
    //                     break; //remove when actual AI Implemented
    ...
    public static RiskAnalysis analyze(String tool, String title, String severity, String component) {
        ...
        try {
            ...
            if (aiCallsThisRun >= MAX_AI_CALLS_PER_RUN) {
                System.out.println("Skipping AI call for \"" + title + "\" due to max AI call limit");
                return null;
            }

            // Force AI to respond ONLY with JSON
            String prompt =
                    "You are a DevSecOps security expert.\nAnalyze the vulnerability.\n\n" +
                            "Tool: " + tool + "\n" +
                            "Component: " + component + "\n" +
            ...
            // Build request JSON
            ObjectNode requestBody = mapper.createObjectNode();
            requestBody.put("model", "openai/gpt-oss-120b"); // Groq model
            ArrayNode messages = mapper.createArrayNode();
            ObjectNode message = mapper.createObjectNode();
            message.put("role", "user");
            message.put("content", prompt);
            ...
            String response = Request.post("https://api.groq.com/openai/v1/chat/completions")
                    .addHeader("Authorization", "Bearer " + API_KEY)
                    .addHeader("Content-Type", "application/json")
                    .bodyString(body, ContentType.APPLICATION_JSON)
                    .execute()
